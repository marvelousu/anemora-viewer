...
- **Build duration**: 5-10 min. Hard timeout is 20 min on free tier; the default build tracks the latest branch per prefix (`MAX_BRANCHES_PER_PREFIX=1`) and keeps only `work/*` as full content (`FULL_CONTENT_PREFIXES=work`) to keep review propagation inside that limit. `wip/*` branches are hydrated from Markdown plus R2 review/devlog files. If approaching the timeout, reduce `originals/` size (e.g., generate webp originals via sharp instead of copying raw PNG — see SPEC §5.1 future tweaks).
- **R2 review cap**: `scripts/setup-r2-images.mjs` fetches only the latest 8 R2 review cycles per branch by default (`R2_MAX_REVIEW_CYCLES_PER_BRANCH=8`). Set `R2_MAX_REVIEW_CYCLES_PER_BRANCH=0` only for archival rebuilds; normal push-triggered deploys should stay capped so new review cycles propagate before the Cloudflare Pages timeout.
