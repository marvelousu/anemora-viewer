...
> 2026-05-24 〜 2026-05-25 にかけて、ユーザー (作者) と Claude Code の対話だけで「iPhone から個人ゲーム開発プロジェクトの進捗を外出中にブラウジングできる PWA」をゼロから作って Cloudflare Pages にデプロイ、AGENTS.md と PR check の運用ルール明文化まで終えた一連の記録。後日ブログ化する素材として残す。
...
> githubのスマホアプリでAnemoraの各セッションの進捗確認を外出中に行なっていますが、アプリ版はファイルのソートも画像ファイルのスライドによるプレビューもできず非常に使いずらいです。解決できるアプリなど作れますか？まずはプランモードで手法を検討したいです。
...
## 13. 想定読者へのメモ (ブログ化するなら)

このプロジェクトの面白さは「何を作ったか」より「**どう作ったか**」の方にある:

- 個人開発者 1 人 + Claude Code (Opus 4.7) の対話だけで、要件定義 → 実装 → デプロイ → 動作確認 → 運用ルール明文化 → 全 branch 配布 → PC 対応、まで 2 日で完走
- iOS の touch / PWA / Pointer Events / bfcache の罠を順に潰した記録としても価値あり
- 並列で Codex セッションが同じ repo に作業中、cherry-pick で衝突を吸収しつつ運用ルール周知できる
- 「Anemora プロジェクトが動いている証左」として viewer URL を GitHub README から能動的に共有、しかし検索エンジンには載せない —「公開度」のグラデーションを設計した
