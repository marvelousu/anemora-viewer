...
iPhone / PC 向けの閲覧専用 PWA。Unity HD-2D 探索アドベンチャー [`marvelousu/anemora`](https://github.com/marvelousu/anemora) の各 `work/*` branch の進捗 (`docs/review/<日時>/` 画像、`docs/devlog/*.md`、Markdown ドキュメント) を、外出中の iPhone やデスクトップから一画面で追えるように作ったツールです。
...
- **branch dropdown** で複数 work branch を切り替え、`work/*` かつ 30 日以内 commit のものを自動取得
...
`npm run build` は `marvelousu/anemora` を `content/` に clone し、`work/*` branch から画像と md を抽出してサムネを生成します。詳細は [SPEC.md](./SPEC.md) §5.1。
