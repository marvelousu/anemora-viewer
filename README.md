...
`npm run build` は `marvelousu/anemora` を `content/` に clone し、`work/*` / `wip/*` branch から画像と md を抽出してサムネを生成します。既定では build size を抑えるため prefix ごとの最新 1 branch を対象にし、`work/*` は full content、`wip/*` は Markdown + R2 review/devlog files の軽量 content として扱います。R2 review 画像は branch ごとの最新 8 cycle だけを取得します (`R2_MAX_REVIEW_CYCLES_PER_BRANCH=0` で無制限)。詳細は [SPEC.md](./SPEC.md) §5.1。
